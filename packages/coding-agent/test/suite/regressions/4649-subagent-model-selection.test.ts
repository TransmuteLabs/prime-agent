...
import type { HostRequestHandlers } from "../../../src/core/kernel/index.ts";
...
describe("ENG-4649 subagent model selection", () => {
	...
	it("omits providers whose credentials are marked expired", async () => {
		...
		try {
			...
			// The stored credential was the only live auth source, so a second call has
			// nothing left to mark.
			expect(harness.session.modelRegistry.markProviderAuthStale(provider)).toBe(false);
	...
	it("includes private Prime models authorized for the selected team", async () => {
		// The team catalog request is served by the mock below.
		allowNetwork();
	...
	it("rejects unavailable requested models at admission without fallback", async () => {
		...
		try {
			...
			// Admission resolves through the executable set, which narrows the available
			// models further by each provider's own catalog.
			const availability = vi
				.spyOn(harness.session.modelRegistry, "getExecutableModels")
				.mockResolvedValue([harness.getModel("parent-model")!]);
			await expect(
