...
import type { GoalHostResponse } from "../../src/core/goals.ts";
...
describe("AgentSession goals", () => {
	...
	it("does not continue when a terminal error reaches the continuation hook", async () => {
		...
		// The merged loop decides continuation in shouldStopAfterTurn (it enqueues follow-ups
		// instead of returning messages), so a terminal error must stop the turn outright.
		const stop = await harness.session.agent.shouldStopAfterTurn?.({
...
describe("initial goal seeding from config", () => {
	...
	it("seeds an active goal from initialGoal config on a fresh top-level session", async () => {
		...
		const { GOAL_STATE_CUSTOM_TYPE } = await import("../../src/core/goals.ts");
	...
	async function createRestartSession(harness: Harness): Promise<AgentSession> {
		...
		const newRegistry = await createInMemoryModelRegistry(newAuth);
		const newSettings = SettingsManager.inMemory();

		const newAgent = new Agent({
			getApiKey: () => "faux-key",
			// The reopened session never runs a turn; the stream is only required by the ctor.
			streamFn: () => {
				throw new Error("The reopened session must not stream");
			},
			initialState: {
				model,
