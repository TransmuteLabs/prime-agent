...
import type { AgentCronJob } from "../src/core/cron-jobs.ts";
import type { AutocompleteProviderFactory } from "../src/core/extensions/types.ts";
import { emptyGoalState, type GoalState } from "../src/core/goals.ts";
import { KeybindingsManager } from "../src/core/keybindings.ts";
import { createSessionSlashCommandMessage, createSessionSlashCommandResultMessage } from "../src/core/messages.ts";
import type { ModelRegistry } from "../src/core/model-registry.ts";
import { PRIME_INFERENCE_PROVIDER_ID } from "../src/core/prime-inference-auth.ts";
import { SettingsManager } from "../src/core/settings-manager.ts";
import { emptyUsage } from "../src/core/usage.ts";
import { InProcessAgentConnection } from "../src/modes/agent-connection/in-process-agent-connection.ts";
import type {
...
import { AgentConnectionPromptAdmissionError } from "../src/modes/agent-connection/types.ts";
import { AgentActivityTracker } from "../src/modes/interactive/agent-activity.ts";
import type { AuthenticationResult } from "../src/modes/interactive/auth-flows.ts";
import { AgentMessageComponent } from "../src/modes/interactive/components/agent-message.ts";
import { BashExecutionComponent } from "../src/modes/interactive/components/bash-execution.ts";
import type { ConfigurationMenuComponent } from "../src/modes/interactive/components/configuration-menu.ts";
import type { AuthSelectorProvider } from "../src/modes/interactive/components/oauth-selector.ts";
import type { ToolExecutionComponent } from "../src/modes/interactive/components/tool-execution.ts";
import { formatSplashCwd, InteractiveMode, truncatePathMiddle } from "../src/modes/interactive/interactive-mode.ts";
import { ClientPromptStashStore, type PromptStashState } from "../src/modes/interactive/prompt-stash-state.ts";
...
describe("InteractiveMode update notifications", () => {
	...
	test("shows the slash update command in one compact line", () => {
		...
		InteractiveMode.prototype.showNewVersionNotification.call(fakeThis, { version: "1.2.3" });
	...
	test.each([true, false])(
		...
		async (supportsSnapshotRestore) => {
			...
			Object.assign(fakeThis, {
				...
				showStatus: vi.fn(),
				// Replacing the editor disposes any open selector; the real method runs against the
				// fake's empty selector state rather than a stub that could hide the call.
				disposeActiveSelector: (InteractiveMode.prototype as unknown as { disposeActiveSelector(): void })
...
describe("InteractiveMode MCP command", () => {
	...
	function withAuthStorage<T extends { authStorage: unknown }>(modelRegistry: T): { modelRegistry: T } {
	...
	test("preserves the explicit /mcp list status output", async () => {
		const fakeThis = {
			...withAuthStorage({ authStorage: { get: vi.fn(() => undefined) } }),
	...
	function createRenderedMcpHarness(manager: SettingsManager) {
		...
		return {
			...withAuthStorage({ authStorage: { get: vi.fn(() => undefined), removeVerified: vi.fn() } }),
...
describe("InteractiveMode connection events", () => {
	test("rendering a switched session tolerates a transient queue refresh failure", async () => {
		const harness = {
			...
			renderSessionStateNow: (): Promise<void> =>
				(
					InteractiveMode.prototype as unknown as { renderSessionStateNow(this: typeof harness): Promise<void> }
	...
	test("clears extension UI when a connection-backed session is replaced", async () => {
		...
		expect(applySnapshotOrder).toBeLessThan(rebindOrder);
		expect(fakeThis.applyConnectionStateSnapshot).toHaveBeenCalledWith(state);
		// The replacement transcript is rendered and subscribed inside the rebind, ahead of the
		// bind whose session_start handlers may send messages (#5943).
		expect(fakeThis.rebindCurrentSession).toHaveBeenCalledWith({ renderBeforeBind: true });
	...
	test("drops a resync superseded while its command catalog refreshes", async () => {
		...
		// The replacement's transcript render happens inside rebindCurrentSession.
	...
	test("renderCurrentSessionState waits for replacement handling before rendering", async () => {
		...
		const fakeThis = {
			...
			renderSessionStateNow: (): Promise<void> =>
				(
					InteractiveMode.prototype as unknown as { renderSessionStateNow(this: typeof fakeThis): Promise<void> }
				).renderSessionStateNow.call(fakeThis),
		};
...
describe("InteractiveMode model selection persistence", () => {
	...
	test("keeps cached daemon models visible when scoped models are active", async () => {
		...
		// An active model scope bounds Ctrl+P cycling, not /model: both the scoped model and the
		// cached catalog resolve by name (ENG-4575).
		await expect(fakeThis.findExactModelMatch("catalog")).resolves.toEqual(catalogModel);
		await expect(fakeThis.findExactModelMatch("scoped")).resolves.toEqual(scopedModel);
...
class EventEmittingReplacementRuntime {
	...
	constructor(session: AgentSessionRuntime["session"], replacement: AgentSessionRuntime["session"]) {
...
describe("InteractiveMode session switch command catalog", () => {
	test.each(["switchSession", "newSession", "fork"] as const)(
		...
		async (operation) => {
			...
			// The replacement resolves its command catalog first, then renders the new transcript.
			expect(calls).toEqual(["catalog", "reset", "render", "reset", "render"]);
...
describe("InteractiveMode.createBaseAutocompleteProvider", () => {
	type AutocompleteHarness = {
		getCachedModelCandidates: () => Array<{ id: string; provider: string; name: string }>;
		getScopedModelState: () => Array<{ model: { id: string; provider: string; name: string } }>;
		createAuthFlows: () => { getLoginProviderOptions: () => AuthSelectorProvider[] };
		currentModelSupportsFastMode: () => boolean;
		getThinkingLevelCompletions: () => null;
		getAvailableThinkingLevels: () => string[];
		getHeartbeatArgumentCompletions: () => null;
		getAutocompleteSourceLabel: () => undefined;
		connectionCommands: [];
	...
	const createBaseAutocompleteProvider = (
		InteractiveMode as unknown as {
			prototype: { createBaseAutocompleteProvider(this: AutocompleteHarness): AutocompleteProvider };
		}
	).prototype.createBaseAutocompleteProvider;

	function createAutocompleteHarness(overrides: Partial<AutocompleteHarness> = {}): AutocompleteHarness {
		return {
	...
	test("matches model command arguments across provider/model order", async () => {
		const fakeThis = createAutocompleteHarness({
			getCachedModelCandidates: () => [
	...
	test("offers the whole catalog for model command arguments while a scope is active", async () => {
		const scopedModel = { id: "gpt-5.5", provider: "openai-codex", name: "GPT-5.5" };
		const fakeThis = createAutocompleteHarness({
			getCachedModelCandidates: () => [
				{ id: "gpt-5.2-codex", provider: "github-copilot", name: "GPT-5.2 Codex" },
				scopedModel,
			],
			getScopedModelState: () => [{ model: scopedModel }],
		});
		...
		// The scope is the Ctrl+P cycling set (docs/usage.md); /model still reaches every model.
		expect(suggestions?.items.map((item) => item.value)).toEqual([
			"github-copilot/gpt-5.2-codex",
			"openai-codex/gpt-5.5",
		]);
...
"[Extensions]
  ...
  path
	...
	test("formats a multi-line skill warning without path noise", () => {
		...
		const output = normalizeRenderedOutput(fakeThis.loadedResourcesContainer, 100);
