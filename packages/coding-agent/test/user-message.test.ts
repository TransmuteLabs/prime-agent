...
describe("UserMessageComponent", () => {
	...
	test("chains Markdown transformers with user message context", () => {
		initTheme("dark");
		const calls: string[] = [];
		const component = new UserMessageComponent("The input is $x^2$.", undefined, undefined, {
			outputPad: 1,
	...
	test("reapplies Markdown transformers when invalidated", () => {
		initTheme("dark");
		let suffix = "before";
		const component = new UserMessageComponent("Message", undefined, undefined, {
			outputPad: 1,
			markdownTransformers: [(markdown) => `${markdown} ${suffix}`],
		});
