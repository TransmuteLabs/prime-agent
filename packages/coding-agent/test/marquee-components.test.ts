...
import { type Component, setKeybindings, TuiMainScreen, visibleWidth } from "@earendil-works/pi-tui";
...
function createFakeTui(): TuiMainScreen {
	return {
		requestRender: () => {},
	} as unknown as TuiMainScreen;
}
...
describe("marquee TUI components", () => {
	...
	test("renders assistant thinking as quiet text without background styling", () => {
		...
		// The tui markdown renderer keeps fence markers as the code-block delimiter; what matters
		// here is that the trace stays quiet text, which the background-escape assertion covers.
