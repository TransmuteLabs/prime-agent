...
import { AgentSession, type RlmChildAgentSnapshot } from "../src/core/agent-session.ts";
import { AuthStorage } from "../src/core/auth-storage.ts";
import type { LoadExtensionsResult } from "../src/core/extensions/index.ts";
import { type HostRequestHandlers, ReplKernelManager } from "../src/core/kernel/index.ts";
...
} from "../src/core/rlm-runtime.ts";
import { SessionManager } from "../src/core/session-manager.ts";
import { SettingsManager, type SettingsStorage } from "../src/core/settings-manager.ts";
import type { Skill } from "../src/core/skills.ts";
import { createSyntheticSourceInfo } from "../src/core/source-info.ts";
import type { BashOperations } from "../src/core/tools/bash.ts";
import { type ActiveSessionState, resolveActiveSessionState } from "../src/modes/daemon/active-session-state.ts";
...
describe("AgentSession rlm recursion", () => {
	...
	// The registry wraps an async-built ModelRuntime; build it once per test so
	// createSession() can stay synchronous for its ~50 call sites. The runtime API
	// key lives in memory, so the registry must hold the same AuthStorage instance.
	...
	beforeEach(async () => {
		...
		modelRegistry = await createModelRegistry(authStorage, join(tempDir, "models.json"));
	});
...
describe("AgentSession RLM session dir", () => {
	...
	let session: AgentSession | undefined;
	// The registry wraps an async-built ModelRuntime; build it once per test so
	// createSession() can stay synchronous. The runtime API key lives in memory,
	// so the registry must hold the same AuthStorage instance the session uses.
	let authStorage: AuthStorage;
	let modelRegistry: ModelRegistry;
