...
import { createAgentConnectionToolDefinition } from "../src/modes/agent-connection/tool-definition.ts";
import { ToolExecutionComponent } from "../src/modes/interactive/components/tool-execution.ts";
import { initTheme, theme } from "../src/modes/interactive/theme/theme.ts";
import { getWorkingPulseFrame, workingIconFrame } from "../src/modes/interactive/theme/working-icon.ts";
...
describe("ToolExecutionComponent parity", () => {
	...
	test("bash renderer does not duplicate final full output truncation details", async () => {
		...
		// The panel indents its rows, so the shape asserted here is "one blank row, then the notice".
