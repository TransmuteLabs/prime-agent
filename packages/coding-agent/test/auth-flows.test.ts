...
import { ProviderAuthFlows, type ProviderAuthFlowsHost } from "../src/modes/interactive/auth-flows.ts";
...
function createHost(authStorage: AuthStorage): {
	...
	const modelRegistry = {
		authStorage,
		// The merged registry refresh is async and reports per-provider outcomes; the login
		// flow refreshes the catalog in the background and inspects that result.
	...
	return {
		host: {
			...
			showWarning: (message) => warningMessages.push(message),
...
describe("ProviderAuthFlows", () => {
	...
	it("stores a reused Prime CLI key when Prime CLI config sync is disabled", async () => {
		...
		expect(result.status).toBe("success");
		expect(fetchMock).toHaveBeenCalledOnce();
		expect(authStorage.getApiKey(PRIME_INFERENCE_PROVIDER_ID)).toBe("prime-cli-key");
