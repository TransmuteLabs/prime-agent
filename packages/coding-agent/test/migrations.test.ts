...
import { migrateLegacySessionDirsToSessionRoot, migrateSessionsFromAgentRoot } from "../src/migrations.ts";
...
describe("session migrations", () => {
	...
	it("leaves root files whose header is not a session", () => {
		const agentDir = mkdtempSync(join(tmpdir(), "prime-agent-migrations-"));
		tempDirs.push(agentDir);
		process.env[ENV_AGENT_DIR] = agentDir;

		const strayFile = join(agentDir, "not-a-session.jsonl");
		writeFileSync(strayFile, `${JSON.stringify({ type: "notes", id: "x" })}\n`);
