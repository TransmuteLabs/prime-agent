...
import type * as DaemonUpdateRestartModule from "../src/cli/daemon-update-restart.ts";
import {
...
import type { AgentSessionRuntimeMetadata } from "../src/core/agent-session-runtime.ts";
import { DAEMON_PROTOCOL_VERSION, DAEMON_SCHEMA_ID } from "../src/modes/daemon/daemon-protocol.ts";
import type * as DaemonSocketModule from "../src/modes/daemon/daemon-socket.ts";
import {
	handlePackageCommand,
	prepareDaemonUpdateRestart,
	runDaemonUpdateRestartCoordinator,
} from "../src/package-manager-cli.ts";
...
vi.mock("../src/cli/daemon-update-restart.ts", async (importOriginal) => {
...
vi.mock("../src/modes/daemon/daemon-socket.ts", async (importOriginal) => ({
	...(await importOriginal<typeof DaemonSocketModule>()),
	defaultDaemonSocketPath: () => mockState.socketPath,
}));

vi.mock("../src/modes/daemon/daemon-supervisor-ownership.ts", () => ({
...
vi.mock("../src/cli/daemon-launch.ts", () => ({
...
describe("self-update daemon restart", () => {
	...
	beforeEach(() => {
		...
		// The release lookup is offline-gated; the fetch stub below stands in for the manifest.
		allowNetwork();
		vi.stubGlobal(
			"fetch",
