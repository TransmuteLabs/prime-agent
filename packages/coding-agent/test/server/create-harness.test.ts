...
describe("coding-agent Harness construction", () => {
	...
	test("preserves coding-agent RLM doctrine and guideline order", () => {
	...
	test("builds each default system prompt from current Harness tool metadata", async () => {
		...
		try {
			...
			try {
				...
				expect(initialPrompt).toContain("You are a general purpose agent that uses code to solve tasks.");
				expect(initialPrompt).toContain("Use read to examine files instead of cat or sed.");
				expect(initialPrompt).toContain(
					"You can inspect PI_* environment variables for current model and session details.",
				);
				expect(initialPrompt).toContain("Use edit for precise changes");
				expect(initialPrompt).toContain("Use write only for new files or complete rewrites.");
				...
				expect(writePrompt).not.toContain("Use read to examine files");
				expect(writePrompt).not.toContain("You can inspect PI_* environment variables");
				...
				expect(readPrompt).toContain("Use read to examine files instead of cat or sed.");
				...
				// Default RLM doctrine does not list tool snippets; guidelines still append.
				expect(inspectPrompt).not.toContain("- inspect:");
	...
	test("omits active custom tools without prompt metadata from the RLM prompt body", () => {
		const prompt = buildCodingAgentHarnessSystemPrompt({
			cwd: "/workspace",
	...
	test("builds the default prompt from RLM doctrine and resolved prompt resources", () => {
		const prompt = buildCodingAgentHarnessSystemPrompt({
			cwd: "/workspace",
			tools: defaultPromptTools,
