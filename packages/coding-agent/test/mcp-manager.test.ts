...
describe("McpManager", () => {
	...
	it("keeps MCP providers registered after the MCP OAuth registry is reset", () => {
		const manager = new McpManager({ authStorage });
		resetMcpOAuthProviders();
		manager.refresh(); // a reset must not leave built-in integrations deauthorized
		expect(getMcpOAuthProvider("mcp:linear")).toBeDefined();
		expect(getMcpOAuthProvider("mcp:notion")).toBeDefined();
	});

	it("re-registers user-declared OAuth servers after the MCP OAuth registry is reset", () => {
	...
	it("keeps the built-in provider when a user server uses a reserved catalog name", () => {
		...
		const provider = getMcpOAuthProvider("mcp:linear");
