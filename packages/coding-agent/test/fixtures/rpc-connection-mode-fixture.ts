import type { AgentConnection, AgentConnectionEventListener } from "../../src/modes/agent-connection/types.ts";
import { runRpcModeWithConnection } from "../../src/modes/rpc/rpc-mode.ts";
	...
	async prompt(message: string) {
		if (message === "async-eof") {
			activePrompt = (async () => {
				...
				await listener({ type: "session_event", event: { type: "agent_end", messages: [], willRetry: false } });
			})();
			return;
		}
