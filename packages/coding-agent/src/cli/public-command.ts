...
import { APP_NAME, SELF_UPDATE_INTERACTIVE_CHILD_ENV } from "../config.ts";
import { AuthStorage } from "../core/auth-storage.ts";
import type { InlineExtension } from "../core/extensions/types.ts";
import { runMcpManagementCommand } from "../core/mcp/mcp-command.ts";
import { SettingsManager } from "../core/settings-manager.ts";
import { handlePackageCommand, isSelfUpdateSource } from "../package-manager-cli.ts";
import { INTERNAL_RUNTIME_COMMAND_MARKER, parseArgs } from "./args.ts";
import {
...
import { runPs, runReap, runShutdownAll } from "./daemon-ps.ts";
import { DAEMON_UPDATE_RESTART_COORDINATOR_FLAG } from "./daemon-update-restart.ts";
...
export interface PublicCommandOptions {
	/** Forwarded to package commands so their project_trust hooks still run. */
	extensionFactories?: InlineExtension[];
}

export async function handlePublicCommand(
	args: string[],
	options: PublicCommandOptions = {},
): Promise<PublicCommandResult> {
	try {
		return await runPublicCommand(args, options);
	} catch (error) {
		return fail(error instanceof Error ? error.message : String(error));
	}
}

async function runPublicCommand(args: string[], options: PublicCommandOptions): Promise<PublicCommandResult> {
	...
	if (command === "update" && process.env[SELF_UPDATE_INTERACTIVE_CHILD_ENV] === "1") {
		await handlePackageCommand(args, options);
		return HANDLED;
	}
	...
	switch (command) {
		...
		case "package":
			return runPackage(args.slice(1), options);
		case "mcp":
		...
		case "update": {
			...
			const updateOptions = parseBooleanOptions(rest, new Set(["--force"]), "update");
			if (!updateOptions) return HANDLED;
			await handlePackageCommand(["update", "--self", ...updateOptions], options);
			return HANDLED;
		}
		case "model":
...
async function runPackage(args: string[], options: PublicCommandOptions): Promise<PublicCommandResult> {
	const subcommand = args[0];
	...
	if (subcommand === "update") {
		...
		await handlePackageCommand(["update", ...(rest.length === 0 ? ["--extensions"] : rest)], options);
		return HANDLED;
	}
	await handlePackageCommand([subcommand, ...rest], options);
	return HANDLED;
}
