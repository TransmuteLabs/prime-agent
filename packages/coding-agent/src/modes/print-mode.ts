...
import { type AgentAutonomousStatus, type AutonomousLimitReason, autonomousLimitReason } from "../core/autonomous.ts";
...
import { latestAutonomousGateAttempt, selectHeadlessTerminalResult } from "./headless-completion.ts";
	...
	try {
		...
		unsubscribe = connection.subscribe((event) => {
			if (mode === "json" && event.type === "session_event") {
				writeRawStdout(`${JSON.stringify(toJsonEvent(event.event))}\n`);
			}
			if (event.type === "extension_error") {
				console.error(`Extension error (${event.extensionPath}): ${event.error}`);
			}
		});
		unsubscribeBackpressure = hooks.subscribeBackpressure?.();
		await hooks.bindHeadlessExtensions?.();
