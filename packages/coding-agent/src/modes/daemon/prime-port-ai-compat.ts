/**
 * Shims for pi-ai symbols that prime-agent's daemon mode expects but pi-ai 0.84
 * does not export: ServiceTier, the structured logger, and a root-level
 * completeSimple. The logger mirrors prime-agent's pi-ai log.ts semantics
 * (injectable sink, console.error fallback for warn/error, never throws).
 */
import type { Api, AssistantMessage, Context, Model, SimpleStreamOptions } from "@earendil-works/pi-ai";
import { completeSimple as completeSimpleFromCompat } from "@earendil-works/pi-ai/compat";
...
export async function completeSimple<TApi extends Api>(
	...
	return completeSimpleFromCompat(model, context, options);
}
