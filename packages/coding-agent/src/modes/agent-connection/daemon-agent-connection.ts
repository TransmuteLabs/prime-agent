...
export class DaemonAgentConnection implements AgentConnection {
	...
	async reload(options?: { beforeSessionStart?: () => void | Promise<void> }): Promise<void> {
		// The hook is a local callback, so it cannot travel with the command. Running it before
		// the request keeps the ordering the interface promises: the worker only emits
		// session_start once the reload it has not yet received completes.
