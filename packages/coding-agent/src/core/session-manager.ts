...
import { APP_NAME, getAgentDir as getDefaultAgentDir, getSessionDirEnvOverride, getSessionsDir } from "../config.ts";
import { readLinesAsBuffers } from "../utils/file-lines.ts";
...
export interface SessionHeader {
	...
	/** Recursive agent nesting depth persisted at session creation (prime-agent). */
	rlmDepth?: number;
...
export interface CompactionEntry<T = unknown> extends SessionEntryBase {
	...
	fromHook?: boolean;
	/** User instructions that shaped this summary; replayed when the summary is rebuilt. */
...
/** Flat session node - entry plus resolved label, without tree structure. */
...
export interface SessionInfo {
	...
	/** Nested RLM depth. Always resolved by the scanner, so consumers can bucket without a guard. */
	rlmDepth: number;
...
/** Session files are named by id alone; uuidv7 ids already sort by creation time. */
...
function getSessionContextSettings(
	path: SessionEntry[],
): Pick<SessionContext, "thinkingLevel" | "serviceTier" | "model"> {
	...
	return { thinkingLevel, serviceTier, model };
}
...
export function buildSessionContext(
	...
	const path = buildSessionPath(entries, leafId, byId);
	const { thinkingLevel, serviceTier, model } = getSessionContextSettings(path);
	const contextEntries = buildContextEntries(entries, leafId, byId);
	const compactionEntry = contextEntries[0]?.type === "compaction" ? contextEntries[0] : undefined;
	// buildContextEntries emits [compaction, ...kept, ...entries after it]; only the
	// kept window precedes the compaction in the full path, and its message count is
	// what the summary reports as retained.
	const keptIds = compactionEntry
		? new Set(
				path
					.slice(
						0,
						path.findIndex((entry) => entry.id === compactionEntry.id),
					)
					.map((entry) => entry.id),
			)
		: undefined;
	...
	for (const entry of contextEntries) {
		...
		if (keptIds?.has(entry.id)) retainedMessageCount += entryMessages.length;
		messages.push(...entryMessages);
	}
	if (compactionEntry) {
		messages[0] = createCompactionSummaryMessage(
			compactionEntry.summary,
			compactionEntry.tokensBefore,
			compactionEntry.timestamp,
			compactionEntry.customInstructions,
 ...
 * A configured session root is a flat store shared by every cwd, so it replaces
 * the per-cwd partition rather than containing it.
 */
/**
 * Sessions are stored flat in one directory and separated by the `cwd` in each file's header,
 * not by a cwd-encoded subdirectory: a session that moves between working directories keeps its
 * file, and cross-project listing reads one directory.
 */
function getDefaultSessionDirPath(_cwd: string, agentDir: string = getDefaultAgentDir()): string {
	return getSessionsDir(agentDir);
}

/**
 * Whether a session directory can hold sessions from other working directories.
 * Only the unconfigured default is cwd-partitioned by construction.
 */
function sessionDirMixesCwds(_dir: string, _cwd: string): boolean {
	// Every session directory is flat, so any of them may hold sessions from other cwds.
	return true;
}
...
/**
 * Session id encoded in a session file name. Files are written as `<sessionId>.jsonl`, but a
 * pre-migration name carries a `<timestamp>_` prefix whose ISO timestamp has `:` and `.`
 * replaced by `-`, so it never contains `_` and the id is everything after the first one.
 */
export function getSessionIdFromFileName(sessionFile: string): string {
	const name = basename(sessionFile).replace(/\.jsonl$/, "");
	const separator = name.indexOf("_");
	return separator === -1 ? name : name.slice(separator + 1);
}

/** {@link getSessionArtifactPath} from a session file; the id defaults to the name's id part. */
export function getSessionArtifactPathForFile(sessionFile: string, sessionId?: string): string {
	return getSessionArtifactPath(dirname(sessionFile), sessionId ?? getSessionIdFromFileName(sessionFile));
}
...
/**
 * Async loader for the daemon: yields while parsing so a large load doesn't freeze
 * other sessions on the event loop. Large files stream line-by-line to avoid retaining
 * both the full input and the parsed entry graph at the same time.
 */
export async function loadEntriesFromFileAsync(
	...
	for await (const line of readLinesAsBuffers(filePath)) {
		const entry = parseSessionEntryLine(line.toString("utf8"));
		if (entry) entries.push(entry);
...
async function listSessionsFromDir(
	...
	try {
		...
		// Only place that evicts readSessionInfo's cache: without it the map grows
		// for the daemon's lifetime, holding a capped-but-large allMessagesText per path.
...
export class SessionManager {
	...
	private _setSessionFile(sessionFile: string, preloadedFileEntries?: FileEntry[]): void {
		...
		if (existsSync(this.sessionFile)) {
			...
			// Sessions written before the depth was persisted resolve it once, on
			// load, so later reads do not re-walk the parent chain from disk.
	...
	newSession(options?: NewSessionOptions): string | undefined {
		...
		// An explicit `rlmDepth: undefined` means "depth unknown" and must not be
		// re-derived, so presence is tested rather than the value.
		...
		if (options?.parentSession && !hasExplicitRlmDepth) {
			try {
				parentHeader = readSessionHeader(options.parentSession) ?? undefined;
			} catch {
	...
	appendCompaction<T = unknown>(
		...
		const resolvedCustomInstructions =
			typeof usageOrCustomInstructions === "string" ? usageOrCustomInstructions : customInstructions;
	...
	getSessionArtifactDir(): string | undefined {
		return this.persist ? getSessionArtifactPath(this.getSessionDir(), this.sessionId) : undefined;
	}
	...
	/**
	 * Give an in-memory session a file on disk, preserving its entries. Returns the
	 * existing path when the session already persists.
	 */
	materializeSessionFile(sessionDir?: string): string {
		if (this.sessionFile) {
			return this.sessionFile;
		}
		const dir = sessionDir ? normalizePath(sessionDir) : this.sessionDir || getDefaultSessionDir(this.cwd);
		if (!existsSync(dir)) {
			mkdirSync(dir, { recursive: true });
		}
		const previousHeader = this.getHeader();
		const timestamp = new Date().toISOString();
		const sessionFile = getSessionFilePath(dir, this.sessionId);
		this.sessionDir = dir;
		...
		const header: SessionHeader = {
			...
			timestamp,
			cwd: this.cwd,
			parentSession: previousHeader?.parentSession ?? undefined,
			rlmDepth: resolveSessionRlmDepth(previousHeader ?? {}, sessionFile),
	...
	static async openAsync(path: string, sessionDir?: string, cwdOverride?: string): Promise<SessionManager> {
		...
		return new SessionManager(cwd ?? process.cwd(), dir, path, true, undefined, entries);
	}
	...
	static continueRecent(cwd: string, sessionDir?: string): SessionManager {
		const dir = sessionDir ? normalizePath(sessionDir) : getDefaultSessionDir(cwd);
		const filterCwd = sessionDirMixesCwds(dir, cwd);
	...
	static forkFrom(
		...
		const newHeader: SessionHeader = {
			...
			rlmDepth: resolveSessionRlmDepth(sourceHeader, resolvedSourcePath),
	...
	static async listAll(
		...
		const envSessionDir = getSessionDirEnvOverride();
		if (envSessionDir) {
			const sessions = await listSessionsFromDir(envSessionDir, progress);
			sessions.sort((a, b) => b.modified.getTime() - a.modified.getTime());
			if (onSession) {
				for (const s of sessions) onSession(s);
			}
		...
		try {
			...
			// Sessions live directly in this directory; the subdirectory walk below only keeps
			// files written under the older cwd-partitioned layout visible.
