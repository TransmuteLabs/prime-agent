...
export class ModelRuntime implements Models {
	...
	setRuntimeApiKey(providerId: string, apiKey: string, options: AuthOperationOptions = {}): Promise<void> {
		...
		return this.enqueueCredentialOperation(providerId, signal, async () => {
			...
			if (this.credentials.underlyingStore instanceof DefaultAuthStorage) {
				this.credentials.underlyingStore.setRuntimeApiKey(providerId, apiKey);
			}
	...
	removeRuntimeApiKey(providerId: string, options: AuthOperationOptions = {}): Promise<void> {
		...
		return this.enqueueCredentialOperation(providerId, signal, async () => {
			...
			if (this.credentials.underlyingStore instanceof DefaultAuthStorage) {
				this.credentials.underlyingStore.removeRuntimeApiKey(providerId);
			}
	...
	private fallbackAuthStorage: DefaultAuthStorage | undefined;

	/** The persistent auth.json store, for the interactive TUI's synchronous auth API. */
	get authStorage(): DefaultAuthStorage {
		const store = this.credentials.underlyingStore;
		if (store instanceof DefaultAuthStorage) {
			return store;
		}
		this.fallbackAuthStorage ??= DefaultAuthStorage.inMemory();
