...
import type { AcpMcpServerConfig } from "./mcp/acp-mcp-types.ts";
...
interface SubmissionNormalizationPolicy {
	...
	/** Reported to input handlers so an extension can see how a queued message will be delivered. */
	streamingBehavior?: "steer" | "followUp";
...
export interface ModelMutationOptions {
	...
	persist?: boolean;
	/** Wait for extension model_select handlers before resolving. */
...
export class AgentSession {
	...
	// The user's request and the tier actually in force are separate: switching to
	// a model without fast mode clamps the latter, and switching back has to
	// restore the former. Only the preference lives here; the tier in force is
	// agent.state.serviceTier, which is what the agent sends to the provider.
	...
	private _lastQueuedMessagesSnapshot: { steering: string[]; followUp: string[] } = { steering: [], followUp: [] };
	...
	/** Set for everything the agent run executes: tools, extension handlers, the provider callback. */
	private readonly _agentRunContext = new AsyncLocalStorage<true>();
	...
	/** Set only while the settlement notification runs; see {@link _emitAgentSettled}. */
	private _announcingAgentSettled = false;
	...
	/** One controller per in-flight execution: concurrent bash runs must abort and be tracked independently. */
	...
	/**
	 * Extension prompt for the active run, kept with the base it was built on: later turns
	 * rebuild their context from the current base and would otherwise drop the override.
	 */
	private _systemPromptOverride?: { prompt: string; baseSnapshot: string };
	...
	constructor(config: AgentSessionConfig) {
		...
		this._serviceTierPreference = config.serviceTierPreference ?? this.agent.state.serviceTier;
		// A session may open on a model without fast mode; the tier in force is
		// clamped from the start while the saved preference stays intact.
		this.agent.state.serviceTier = this._getEffectiveServiceTier(this._serviceTierPreference);
	...
	private async _getRequiredRequestAuth(model: Model<any>): Promise<{
		...
		// A provider may authenticate by header or bearer token with no key of its own; only a
		// resolution carrying neither is unauthenticated.
		const headers = withoutDeletedHeaders(result.headers);
		if (result.apiKey || (headers && Object.keys(headers).length > 0)) {
			return {
				model: result.baseUrl ? { ...model, baseUrl: result.baseUrl } : model,
	...
	/**
	 * Auth for a summarization request (compaction, branch summary).
	 *
	 * Only the built-in stream function must have registry credentials: a caller-supplied
	 * streamFunction reaches the provider on its own terms, and header- or bearer-only providers
	 * carry no apiKey at all. Both are refused by the request-auth path, which exists for the
	...
	private async _getSummarizationRequestAuth(model: Model<any>): Promise<{
		...
		try {
			return await this._getRequiredRequestAuth(model);
		} catch {
			return { model };
		}
	}
	...
	private _installAgentToolHooks(): void {
		...
		this.agent.afterToolCall = async ({ toolCall, args, result, isError }) => {
			...
			// After the extension hook, so images an extension injected or replaced are normalized too.
	...
	private _installAgentTurnHook(): void {
		...
		this.agent.shouldStopAfterTurn = async (context, signal) => {
			...
			//
			// Continuation is the "agent would otherwise stop" hook, so it may only run on a turn
			// after which the loop has nothing else to do: the loop calls shouldStopAfterTurn after
			// every turn including mid tool loop, and drains follow-ups only once the inner loop is
			// out of tool calls and queued messages. Computing it any earlier spends the goal's
			// continuation budget on a turn the loop was going to take anyway, and injects the
			// continuation message into the middle of a tool loop. Queued steering or follow-up
			// messages own the next turn; the continuation is reconsidered after they are consumed.
			if (!this._steeringStopPending && !context.hasMoreToolCalls && !this.agent.hasQueuedMessages()) {
	...
	private _emitQueueUpdate(): void {
		...
		// queue_update reports a change of the queued messages themselves. Action transitions that
		// leave both queues identical are session_action_update's subject, not this event's.
		...
		if (JSON.stringify(queued) === JSON.stringify(this._lastQueuedMessagesSnapshot)) {
			return;
		}
		this._lastQueuedMessagesSnapshot = queued;
		...
		for (const entry of branch) {
			switch (entry.type) {
				...
				case "service_tier_change":
					continue;
				default:
					// Any message, custom entry (including a persisted thread_goal_state) or
					// other content means the session has been used.
					return false;
			}
		}
		return true;
	...
	private async _validateCanStartAgentRun(): Promise<void> {
		...
		// The snapshot only knows providers configured when it was last refreshed. A credential
		// stored after that is still usable, so a live check decides before the run is refused.
		const hasConfiguredAuth =
			this._modelRegistry.hasConfiguredAuth(this.model) || (await this._modelRegistry.hasResolvableAuth(this.model));
	...
	private async _emitAgentSettled(): Promise<void> {
		// Settlement is announced from the tail of the run that is ending, so the agent's
		// streaming flag and the completing action are still set while handlers run.
		this._announcingAgentSettled = true;
		try {
			try {
			...
			this._announcingAgentSettled = false;
		}
	}
	...
	private async _processAgentEvent(event: AgentEvent): Promise<void> {
		...
		if (event.type === "message_end") {
			...
			if (event.message.role === "assistant") {
				...
				// A truncated response is an unfinished turn like a failed one: clearing the
				// recovery state on it would grant a fresh compact-and-retry attempt every time.
		...
		if (event.type === "agent_end") {
			...
			if (!compactionWillRetry) {
				...
				// The run is over and no continuation follows, so deferred bash output belongs to
				// this run's transcript rather than to whatever prompt comes next.
				this._flushPendingBashMessages();
				this._flushPendingCustomMessages();
				// Work queued during this run (a follow-up from an agent_end handler, a steer)
				// continues the session, so settlement belongs to the turn that ends with an
				// empty queue.
				if (this.queuedActionCount === 0) await this._emitAgentSettled();
			}
		}
	}
	...
	get isIdle(): boolean {
		if (this._announcingAgentSettled) return true;
	...
	private _normalizeSubmission(
		...
		if (policy.inputSource !== undefined && this._extensionRunner.hasHandlers("input")) {
			return this._extensionRunner
				.emitInput(text, images, policy.inputSource, policy.streamingBehavior)
				.then((result) => {
	...
	private _applyPreparedSystemPrompt(
		...
		this._systemPromptOverride =
			hasExtensionPrompt && extensionPrompt !== undefined && preparation !== undefined
				? { prompt: extensionPrompt, baseSnapshot: preparation.basePromptSnapshot }
				: undefined;
		this.agent.state.systemPrompt = this._currentSystemPrompt();
	}

	private _currentSystemPrompt(): string {
		const override = this._systemPromptOverride;
		if (!override) return this._baseSystemPrompt;
		return this._refreshExtensionSystemPrompt(override.prompt, override.baseSnapshot);
	}
		...
		const run = async () => {
			try {
				...
				const normalizationResult = this._normalizeSubmission(text, options?.images, {
					...
					// Only a message that will actually be queued carries a delivery mode.
				...
				// A direct prompt would otherwise wait for admission until manual compaction
				// ends, and a session_before_compact hook can hold that open indefinitely.
				// Callers that opted into queueing keep queueing.
				if (
					this._compactionAbortController !== undefined &&
					options?.queueIfBusy !== true &&
				...
				await result.ticket.completed;
				await this.waitForSessionInputIdle();
				await this._awaitOwnedPostCompactionContinuation();
			} catch (error) {
				reportPreflight(false);
				throw error;
			} finally {
	...
	private _shouldWaitForModelSelectEmit(options: ModelMutationOptions): boolean {
	...
	async compact(customInstructions?: string, options: { skipAbort?: boolean } = {}): Promise<CompactionResult> {
		...
		// The agent subscription stays connected: manual compaction waits for the active run to
		// settle and summary generation emits no agent events, while dropping the subscription
		// loses the abort's own events and lets the settling turn run a second, automatic
		// compaction underneath this one (#7253).
		...
		try {
			...
			const errorMessage = aborted ? undefined : skipped ? message : `Compaction failed: ${message}`;
			this._compactionAbortController = undefined;
			this._emit({
				type: "compaction_end",
				reason: "manual",
	...
	private async _performCompaction(options: {
		...
		const { model, apiKey, headers, env, customInstructions, signal, reason, willRetry } = options;
		// The extension events have no "requested" arm: from an extension's side a
		// user-requested compaction is a manual one.
		const extensionReason = reason === "requested" ? "manual" : reason;
	...
	/**
	 * A run that compaction interrupted is not over until its continuation has run, so a caller
	 * awaiting the prompt must not observe the truncated turn as the final result. Chained
	 * continuations each install a new settlement, so wait until none is outstanding.
	 */
	private async _awaitOwnedPostCompactionContinuation(): Promise<void> {
		while (true) {
		...
		while (this._postCompactionContinuationScheduled && this._postCompactionContinuationSettlement === settlement) {
			...
			try {
				...
				if (this.unfinishedActionCount > 0 || this._sessionInputPumpRequested) {
					...
					this._postCompactionContinuationScheduled = false;
					continuation = this._agentRunContext.run(true, () => this.agent.continue());
				}
			} finally {
				commitFence.release();
			}
	...
	private async _reviewAutoRefine(context: AutoRefineReviewRequest, signal?: AbortSignal): Promise<AutoRefineReview> {
		...
		const { model: requestModel, apiKey, headers, env } = await this._getRequiredRequestAuth(model);
	 ...
	 * A model-requested compaction (compact skill) runs even with auto-compaction off.
	 *
	...
	private _getThresholdContextTokens(
		...
		// For error messages or all-zero usage messages, estimate from the message-size
		// fallback. This ensures sessions that hit persistent API errors (e.g. 529) or
		// malformed zero-usage responses can still compact and do not reset context accounting.
	...
	private async _checkCompaction(
		...
		if (
			...
			(contextOverflow || recoverableLength)
		) {
			...
			if (assistantMessage.stopReason === "stop") {
				return await this._runAutoCompaction("overflow", false, queueAutonomousContinuation);
			}
			...
			if (this._overflowRecovery !== "idle") {
				if (this._overflowRecovery === "attempted") {
					...
					await this._endCompactionUnsuccessfully("overflow", "failed", errorMessage);
				}
				return false;
			}

			this._overflowRecovery = "attempted";
			// Remove the failed or truncated message from agent state, compact, and
			// retry once. The message remains in session history but is excluded from retry context.
			const messages = this.agent.state.messages;
			if (messages.length > 0 && messages[messages.length - 1].role === "assistant") {
				this.agent.state.messages = messages.slice(0, -1);
			}
			return await this._runAutoCompaction("overflow", true, queueAutonomousContinuation);
		}

		if (this._pendingRequestedCompaction !== undefined) {
			return await this._runAutoCompaction("requested", false, queueAutonomousContinuation);
		}
		...
		if (shouldCompact(contextTokens, contextWindow, settings)) {
			...
			return await this._runAutoCompaction("threshold", false, queueAutonomousContinuation);
		}
		return false;
	}
	...
	private async _endCompactionUnsuccessfully(
		...
		// Every unsuccessful compaction end owes extensions a session_compact_failed;
		// emitting it here keeps the auto-compaction paths from going silent.
		await this._emitSessionCompactFailed({
			// The extension event has no "requested" arm: from an extension's side a
			// user-requested compaction is a manual one.
			reason: reason === "requested" ? "manual" : reason,
			errorMessage: options.aborted ? undefined : message,
			aborted: options.aborted ?? false,
			willRetry: false,
	...
	private async _runAutoCompaction(
		...
		try {
			let summarizationAuth: Awaited<ReturnType<typeof this._getSummarizationRequestAuth>> | undefined;
			let authError: string | undefined;
			if (this.model) {
				try {
					summarizationAuth = await this._getSummarizationRequestAuth(this.model);
				} catch (error) {
					authError = error instanceof Error ? error.message : String(error);
				}
			}
			if (!summarizationAuth) {
				const detail = !this.model ? "no model is selected" : (authError ?? "no API key is available");
			...
			const result = await this._performCompaction({
				model: summarizationAuth.model,
				apiKey: summarizationAuth.apiKey,
				headers: summarizationAuth.headers,
				env: summarizationAuth.env,
				customInstructions,
			...
			if (willRetry) {
				...
				// The interrupted response was persisted on message_end before _checkCompaction removed it
				// from agent state, and rebuilding state from the new compaction can restore it as the
				// final message. agent.continue() refuses a trailing assistant, so drop it again — a
				// truncated length response is as unusable for continuation as a failed one.
				const stopReason = lastMsg?.role === "assistant" ? (lastMsg as AssistantMessage).stopReason : undefined;
				if (stopReason === "error" || stopReason === "length") {
					this.agent.state.messages = messages.slice(0, -1);
				}

				if (queueAutonomousContinuation) {
				...
				this._scheduleAutoRefineAfterCompaction(willContinueAfterCompaction);
				return false;
			} else if (queueAutonomousContinuation && (shouldContinueAfterCompaction || hasQueuedMessages)) {
				// Compaction can intentionally stop a tool loop between turns.
				// Queued follow-up/steering/custom messages can also be waiting.
				this._schedulePostCompactionContinue(shouldContinueAfterCompaction);
				this._scheduleAutoRefineAfterCompaction(willContinueAfterCompaction);
				// A continuation is scheduled, so the run is not settled yet.
				return true;
			} else {
				this._scheduleAutoRefineAfterCompaction(willContinueAfterCompaction);
			}
	...
	private _bindExtensionCore(runner: ExtensionRunner): void {
		...
		runner.bindCore(
			...
			},
			{
				...
				// The same object identity across calls: consumers diff prompt options by reference.
			...
			},
			{
				...
				registerNativeProvider: (provider) => {
					this._modelRegistry.registerProvider(provider);
					this._refreshCurrentModelFromRegistry();
				},
	...
	private _refreshToolRegistry(options?: { activeToolNames?: string[]; includeAllExtensionTools?: boolean }): void {
		...
		// The denylist outranks the allowlist: an excluded name stays hidden even when listed.
	...
	async reload(options?: { beforeSessionStart?: () => void | Promise<void> }): Promise<void> {
		...
		// The replaced runtime keeps its event-bus subscriptions and a usable ctx until it is
		// invalidated, so a captured pi would keep firing against the previous session.
	...
	private _createInlineRlmSubagentRuntime(options: CreateRlmSubagentRuntimeOptions): RlmSubagentRuntime {
		...
		const child = new AgentSession({
			...
			// The child inherits the tier the parent already clamped for the child's model; without
			// it the session would fall back to the default tier while its transcript records the
			// inherited one (ENG-4620).
	...
	/**
	 * Retry only errors the provider layer classifies as transient, then subtract the cases this
	 * session knows are permanent regardless of their wording. Retrying by default spends the
	 * budget on errors a repeat request cannot fix, such as a rejected credential.
	 */
	...
	private async _handleRetryableError(
		...
		setTimeout(() => {
			this._agentRunContext.run(true, () => this.agent.continue()).catch(() => {});
		}, 0);
	...
	async navigateTree(
		...
		// Navigation settles the agent before it moves the leaf, so an external caller waits for the
		// active run. A caller running INSIDE that run (a tool, an extension handler, the provider
		// callback) would wait on a turn that cannot finish until it returns: refuse instead of
		// deadlocking.
		if (this._agentRunContext.getStore()) {
	...
	private async _navigateTreeUnderPause(
		...
		try {
			...
			// Billed to the session like any other request, so the entry has to carry it.
	...
	getSessionStats(): SessionStats {
		...
		// Session entries, not live messages: compaction removes assistants from the live view
		// while their tokens were still spent, and compaction/branch_summary/toolResult entries
		// carry usage of their own that never appears on an assistant message.
			...
			for (let i = branchEntries.length - 1; i > compactionIndex; i--) {
				...
				if (entry.type === "message" && entry.message.role === "assistant") {
					...
					if (assistant.stopReason !== "aborted" && assistant.stopReason !== "error") {
						...
						// A zero-usage assistant (a tool-only or empty turn) carries no context
						// measurement, so keep scanning back instead of concluding there is none.
						if (contextTokens > 0) {
	...
	private async _modelGetExecutable(): Promise<Model<any>[]> {
		return this._modelRegistry.getExecutableModels();
	}

	private async _modelCanUse(model: Model<any>): Promise<boolean> {
		return this._modelRegistry.canUseModel(model);
	}

	private _modelGetAuthSourceToken(provider: string): AuthSourceToken | undefined {
		return this._modelRegistry.getProviderAuthSourceToken(provider);
	}

	private _modelMarkAuthSourceStale(token: AuthSourceToken): boolean {
		return this._modelRegistry.markProviderAuthSourceStale(token);
	}

	private _modelMarkAuthStale(provider: string): boolean {
		return this._modelRegistry.markProviderAuthStale(provider);
	}
