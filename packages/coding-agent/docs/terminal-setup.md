...
Prime Agent auto-detects OSC 8 hyperlinks, inline image protocols, and truecolor. If detection fails behind a terminal proxy or multiplexer, use these advanced overrides:
...
Prime Agent uses Windows-style keybindings when running natively on Windows or in WSL:

- `Alt+V` pastes an image from the clipboard.
- `Ctrl+F` searches the transcript in fullscreen mode, and `Ctrl+Up`/`Ctrl+Down` jump between marked messages.
- `Ctrl+Z` undoes editing on native Windows; WSL uses `Alt+Z` so `Ctrl+Z` can suspend the agent.
- `Ctrl+Q` queues a follow-up message.
...
- Windows Terminal binds `Alt+Enter` to fullscreen by default. That prevents Prime Agent from receiving `Alt+Enter` for follow-up queueing (on Windows and WSL the default for that action is `Ctrl+Q` instead).
