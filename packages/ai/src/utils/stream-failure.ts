...
import { appendAssistantMessageDiagnostic, extractDiagnosticError } from "./diagnostics.ts";
...
export function streamFailureFromStopReason(
	...
	extra?: Pick<StreamFailureInfo, "requestId"> & { message?: string },
): StreamFailureError {
	...
	// A caller that already mapped the stop reason to provider text owns the wording; classification
	// is additive here and travels in `info`, so the user-facing message must not be rewritten.
	const message =
		extra?.message ||
		(rawStopReason
