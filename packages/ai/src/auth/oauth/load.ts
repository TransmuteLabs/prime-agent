...
type OAuthFlowLoaders = {
	...
	primeInference: () => OAuthAuth | Promise<OAuthAuth>;
...
export const loadPrimeInferenceOAuth = async (): Promise<OAuthAuth> => {
	if (bundledLoaders) return bundledLoaders.primeInference();
	return ((await importOAuthModule("./prime-inference.ts")) as { primeInferenceOAuth: OAuthAuth }).primeInferenceOAuth;
};
