...
/**
 * Prime Inference team ID for the `X-Prime-Team-ID` request header.
 * PRIME_TEAM_ID env var wins; otherwise falls back to `team_id` in
 * ~/.prime/config.json (written by the Prime CLI or the login flow).
 */
export function getPrimeTeamId(env?: ProviderEnv): string | undefined {
	const fromEnv = getProviderEnvValue("PRIME_TEAM_ID", env);
