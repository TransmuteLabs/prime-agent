...
import { getPrimeTeamId } from "../env-api-keys.ts";
...
function createClient(
	...
	if (model.provider === "prime-inference" && !hasHeader(headers, "x-prime-team-id")) {
		const teamId = getPrimeTeamId(env);
...
function detectCompat(model: Model<"openai-completions">): ResolvedOpenAICompletionsCompat {
	...
	const cacheControlFormat =
		(provider === "openrouter" || isPrimeInference) && model.id.startsWith("anthropic/") ? "anthropic" : undefined;
	...
	return {
		...
		thinkingTokenBudgetField: undefined,
		supportsStrictMode: !isMoonshot && !isTogether && !isCloudflareAiGateway && !isNvidia && !isPrimeInference,
