...
describe("openai-responses provider defaults", () => {
	...
	it("can omit the session_id header while preserving other cache-affinity headers", async () => {
		const proxyModel: Model<"openai-responses"> = {
			...
			provider: "opencode",
			baseUrl: "https://proxy.example.com/v1",
			// Upstream generalized the fork's sendSessionIdHeader boolean into this format enum.
	...
	it.each([
		...
		const sse = `${[
			`data: ${JSON.stringify({
				...
				response: {
					...
					// Stay under the model's long-context cost tier so the assertion below can use the
					// base rates; the multiplier, not tier selection, is what this case pins.
					usage: {
						input_tokens: 100000,
		...
		const perMillion = 100000 / 1000000;
		expect(result.usage.cost.input).toBeCloseTo(model.cost.input * perMillion * multiplier, 10);
		expect(result.usage.cost.output).toBeCloseTo(model.cost.output * perMillion * multiplier, 10);
		expect(result.usage.cost.total).toBeCloseTo((model.cost.input + model.cost.output) * perMillion * multiplier, 10);
	});
});
