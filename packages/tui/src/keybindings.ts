...
export interface KeybindingDefinition {
	...
	description?: string;
	/**
	 * Rebinding a key inside a scope suppresses the same key's DEFAULT bindings
	 * in that scope, so a user remap never silently collides with a default.
	 */
...
export const TUI_KEYBINDINGS = {
	...
	"tui.editor.pageUp": { defaultKeys: ["pageUp", "ctrl+pageUp"], description: "Page up", defaultKeyScope: "editor" },
	"tui.editor.pageDown": {
...
export class KeybindingsManager {
	...
	private rebuild(): void {
		...
		// A key the user ADDED to a binding (not one of its defaults) outranks the
		// same key's defaults elsewhere in the scope; plain defaults do not.
