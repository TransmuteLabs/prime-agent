...
export class TuiAltScreen extends TuiBase implements ViewportTUI {
	...
	private mouseEnableSequence = ENABLE_BUTTON_MOTION_MOUSE;
	...
	protected override beforeTerminalStart(): void {
		...
		this.mouseEnableSequence = mouseSequence;
	...
	/** Toggle mouse tracking off while a visible overlay requests terminal-native selection. */
	protected override overlayStackChanged(): void {
		if (!this.mouseEnabled) return;
		const suspend = this.hasVisibleOverlayWith((options) => options?.suspendFullscreenMouse === true);
		if (suspend === this.mouseSuspended) return;
		this.mouseSuspended = suspend;
		if (!this.altScreenActive) return;
		if (!suspend) {
			this.stopSelectionAutoScroll();
		}
		this.terminal.write(suspend ? DISABLE_MOUSE : this.mouseEnableSequence);
	}
	...
	private handleViewportInput(data: string): { consume?: boolean } | undefined {
		...
		// While an overlay suspends mouse tracking the terminal owns pointer
		// events (native selection); ignore any in-flight sequences.
		if (this.mouseSuspended && this.isMouseSequence(data)) return { consume: true };
